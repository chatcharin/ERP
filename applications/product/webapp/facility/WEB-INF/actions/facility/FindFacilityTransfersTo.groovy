...
Debug.logInfo("-=-=-=-  SERVICE -=-=-=-"+facilityId, "");
Debug.logInfo("-=-=-=-  SERVICE -=-=-=-"+completeRequested, "");
// get the 'to' this facility transfers
if (activeOnly) {
    exprsTo = [EntityCondition.makeCondition("facilityId", EntityOperator.EQUALS, facilityId),
...
if (toTransfers) {
	...
	for(item in toTransfers){
	    Debug.logInfo("-=-=-=-  SERVICE Complete -=-=-=-", "");
	}
}
...
if (completeRequested) {
    exprsFrom = [EntityCondition.makeCondition("facilityIdTo", EntityOperator.EQUALS, facilityId),
                 EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "IXF_REQUESTED")];
}
